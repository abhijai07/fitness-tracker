...
def init_extra_tables():
    with engine.begin() as conn:
        ...
        existing = conn.execute(
            text("SELECT id FROM app_users WHERE username = 'default'")
        ).fetchone()
        ...
        if existing is None:
            ...
            conn.execute(
                text("""
                    INSERT INTO app_profiles (user_id)
                    VALUES (1)
...
def get_user_profile(user_id: int):
    with engine.begin() as conn:
        ...
        row = conn.execute(
            text("SELECT * FROM app_profiles WHERE user_id = :uid"),
            {"uid": user_id},
        ).fetchone()
...
def get_exercise_note(exercise_id: int):
    with engine.begin() as conn:
        row = conn.execute(
            text("SELECT note FROM exercise_notes WHERE exercise_id = :id"),
            {"id": exercise_id},
        ).fetchone()
    ...
    for workout in workouts:
        ...
        for exercise in exercises:
            sets = db.query(models.WorkoutSet).filter(
                models.WorkoutSet.exercise_id == exercise.id
            ).all()
...
def get_daily_totals(user_id: int, db: Session):
    ...
    for meal in meals:
        ...
        daily_totals[date_key]["protein_g"] += meal.protein_g or 0
        daily_totals[date_key]["carbs_g"] += meal.carbs_g or 0
        daily_totals[date_key]["fat_g"] += meal.fat_g or 0
...
def list_users():
    with engine.begin() as conn:
        users = conn.execute(
            text("SELECT id, username, created_at FROM app_users ORDER BY id")
        ).fetchall()
...
def update_meal(meal_id: int, payload: dict, db: Session = Depends(get_db)):
    meal = db.query(models.Meal).filter(models.Meal.id == meal_id).first()

    if meal is None:
        return {"error": "Meal not found"}

    meal.date = payload.get("date", meal.date)
    meal.meal_type = payload.get("meal_type", meal.meal_type)
    meal.food_name = payload.get("food_name", meal.food_name)
    meal.calories = payload.get("calories", meal.calories)
    meal.protein_g = payload.get("protein_g", meal.protein_g)
    meal.carbs_g = payload.get("carbs_g", meal.carbs_g)
    meal.fat_g = payload.get("fat_g", meal.fat_g)
...
def create_workout(workout: dict, db: Session = Depends(get_db)):
    ...
    for exercise in workout.get("exercises", []):
        db_exercise = models.Exercise(
            ...
            exercise_note=exercise.get("exercise_note", ""),
        )
...
def delete_workout(workout_id: int, db: Session = Depends(get_db)):
    ...
    for exercise in exercises:
        ...
        with engine.begin() as conn:
            conn.execute(
                text("DELETE FROM exercise_notes WHERE exercise_id = :id"),
                {"id": exercise.id},
            )
...
def get_exercise_progress(
    ...
    for workout in workouts:
        exercises = db.query(models.Exercise).filter(
            models.Exercise.workout_id == workout.id,
            models.Exercise.exercise_name.ilike(f"%{exercise_name}%"),
        ).all()
...
def get_body_weight_progress(user_id: int, db: Session = Depends(get_db)):
    logs = db.query(models.DailyLog).filter(
        models.DailyLog.user_id == user_id,
        models.DailyLog.body_weight_kg != None,
    ).all()
...
def get_personal_records(user_id: int, db: Session = Depends(get_db)):
        ...
        for exercise in workout["exercises"]:
            ...
            for s in exercise["sets"]:
                ...
                if s["weight"] > prs[name]["max_weight"]:
                    ...
                    prs[name]["best_set"] = (
                        f'{s["weight"]} {s["weight_unit"]} x {s["reps"]}'
                    )
    ...
    return sorted(
        prs.values(),
        key=lambda item: item["max_weight"],
        reverse=True,
    )
...
def get_streaks(user_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "water_streak": calculate_streak([
            log.date for log in logs
            if log.water_liters is not None and log.water_liters >= goals["water_goal"]
        ]),
        "sleep_streak": calculate_streak([
            log.date for log in logs
            if log.sleep_hours is not None and log.sleep_hours >= goals["sleep_goal"]
        ]),
        "steps_streak": calculate_streak([
            log.date for log in logs
            if log.steps is not None and log.steps >= goals["steps_goal"]
        ]),
        "protein_streak": calculate_streak([
            item["date"] for item in meals.values()
            if item["protein_g"] >= goals["protein_goal"]
        ]),
    }
...
def get_weekly_analytics(user_id: int, db: Session = Depends(get_db)):
    ...
    for i in range(7):
        ...
        day_log = next(
            (log for log in logs if str(log.date) == day_key),
            None,
        )
        ...
        for workout in day_workouts:
            ...
            for exercise in workout["exercises"]:
                ...
                for s in exercise["sets"]:
                    day_volume += (s["weight"] or 0) * (s["reps"] or 0)
        ...
        weekly_days.append({
            ...
            "protein_g": day_meals["protein_g"],
            "water_liters": day_log.water_liters if day_log else 0,
            "sleep_hours": day_log.sleep_hours if day_log else 0,
            "steps": day_log.steps if day_log else 0,
        })
    ...
    return {
        ...
        "summary": {
            ...
            "total_volume": total_volume,
            "avg_protein": round(
                sum(day["protein_g"] for day in weekly_days) / 7,
                1,
            ),
            "avg_sleep": round(
                sum(day["sleep_hours"] or 0 for day in weekly_days) / 7,
                1,
            ),
            "avg_water": round(
                sum(day["water_liters"] or 0 for day in weekly_days) / 7,
                1,
            ),
            "avg_steps": round(
                sum(day["steps"] or 0 for day in weekly_days) / 7,
            ),
            "most_trained_muscle": (
                max(muscle_counts.items(), key=lambda item: item[1])[0]
                if muscle_counts
                else "None"
            ),
        },
        "muscle_counts": [
            {"muscle": key, "count": value}
            for key, value in muscle_counts.items()
        ],
    }
        ...
        [
            {
                ...
                "sets": metrics["exercise_sets"][name],
                "sessions": metrics["exercise_frequency"][name],
            }
        ...
        [
            {
                "muscle": name,
                "volume": round(volume, 1),
                "sessions": metrics["muscle_frequency"][name],
            }
...
def get_split_analytics(user_id: int, db: Session = Depends(get_db)):
    ...
    splits = [
        ...
        for key, value in metrics["split_counts"].items()
    ]
    ...
    monthly = [
        ...
        for key, value in metrics["monthly_split_counts"].items()
    ]
    ...
    if metrics["split_counts"]:
        ...
        if upper_count >= 2 and leg_count == 0:
            ...
            warning = (
                "One workout type is dominating your split. "
                "Consider balancing the week."
            )
...
def get_achievements(user_id: int, db: Session = Depends(get_db)):
    ...
    max_weight = max(
        [item["max_weight"] for item in prs],
        default=0,
    )

    protein_days = sum(
        1 for item in meals.values()
        if item["protein_g"] >= goals["protein_goal"]
    )
    ...
    water_days = sum(
        ...
        and log.water_liters >= goals["water_goal"]
    )

    sleep_days = sum(
        1 for log in logs
        if log.sleep_hours is not None
        and log.sleep_hours >= goals["sleep_goal"]
    )

    steps_days = sum(
        1 for log in logs
        if log.steps is not None
        and log.steps >= goals["steps_goal"]
    )
    ...
    definitions = [
        ...
        (
            "Push Specialist",
            "Log 5 push/chest workouts",
            sum(
                value for key, value in metrics["split_counts"].items()
                if "push" in key.lower() or "chest" in key.lower()
            ) >= 5,
        ),
        (
            "Pull Specialist",
...
def get_insights(user_id: int, db: Session = Depends(get_db)):
    ...
    if weekly["summary"]["total_workouts"] >= 4:
        insights.append(
            "Strong workout consistency this week. You trained 4 or more times."
        )
    elif weekly["summary"]["total_workouts"] == 0:
        insights.append(
            "No workouts logged this week yet. Add one workout to restart your streak."
        )
    else:
        insights.append(
            "You have logged workouts this week. Try to reach 3-4 sessions for better consistency."
        )
    ...
    if weekly["summary"]["avg_protein"] >= goals["protein_goal"]:
        ...
        insights.append(
            "Your average protein intake is below your goal. Add more high-protein meals."
        )

    if weekly["summary"]["avg_sleep"] < goals["sleep_goal"]:
        insights.append(
            "Sleep is below your goal. Recovery may improve if you increase sleep."
        )

    if streaks["workout_streak"] >= 3:
        insights.append(
            f'Great job. Your workout streak is {streaks["workout_streak"]} days.'
        )

    ready_muscles = [
        item["muscle"] for item in recovery
    ...
    if leaderboard["exercises"]:
        ...
        insights.append(
            f'Highest total volume exercise: {top["exercise"]} with {top["volume"]} volume.'
        )
...
def export_report(
    ...
    if format.lower() == "txt":
        ...
        lines.append(
            f"Achievements: {achievements['unlocked_count']} / {achievements['total_count']}"
        )
        ...
        for pr in prs[:20]:
            lines.append(
                f"- {pr['exercise_name']}: {pr['best_set']} | Best Volume {pr['best_volume']}"
            )
        ...
        return Response(
            ...
            headers={
                "Content-Disposition": "attachment; filename=fitness_report.txt"
            },
        )
        ...
        for exercise in workout["exercises"]:
            ...
            for s in exercise["sets"]:
                writer.writerow([
                    ...
                    workout["date"],
                    exercise["exercise_name"],
                    f"{s['reps']} reps x {s['weight']} {s['weight_unit']}",
                    s["volume"],
                ])
    ...
    for day in meals.values():
        writer.writerow([
            ...
            f"{day['calories']}/{day['protein_g']}/{day['carbs_g']}/{day['fat_g']}",
        ])
    ...
    return Response(
        ...
        media_type="text/csv",
        headers={
            "Content-Disposition": "attachment; filename=fitness_report.csv"
        },
    )
def get_username(user_id: int):
    with engine.begin() as conn:
        row = conn.execute(
            text("SELECT username FROM app_users WHERE id = :id"),
            {"id": user_id},
        ).fetchone()
...
def social_summary(user_id: int, db: Session):
    ...
    max_pr = max(
        [item.get("max_weight", 0) for item in prs],
        default=0,
    )
...
def add_friend(payload: dict):
    ...
    return {
        "message": f"{friend.username} added as a friend",
        "friend": {
            "id": friend.id,
...
def list_friends(user_id: int, db: Session = Depends(get_db)):
    return [
        social_summary(friend_id, db)
        for friend_id in get_friend_ids(user_id)
    ]
...
def friends_leaderboard(user_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "weekly_volume": sorted(
            rows,
            key=lambda item: item["weekly_volume"],
            reverse=True,
        ),
        "streaks": sorted(
            rows,
            key=lambda item: item["workout_streak"],
            reverse=True,
        ),
        "prs": sorted(
            rows,
            key=lambda item: item["max_pr"],
            reverse=True,
        ),
        "workouts": sorted(
            rows,
            key=lambda item: item["total_workouts"],
            reverse=True,
        ),
    }
...
def compare_friend(
    ...
    user_prs = {
        ...
        for item in get_personal_records(user_id, db)
    }

    friend_prs = {
        item["exercise_name"]: item
        for item in get_personal_records(friend_id, db)
    }

    exercise_names = sorted(
        set(user_prs.keys()) | set(friend_prs.keys())
    )
    ...
    return {
        ...
        "summary_chart": [
            {
                "metric": "Total Volume",
                "you": user_summary["total_volume"],
                "friend": friend_summary["total_volume"],
            },
            {
                "metric": "Weekly Volume",
                "you": user_summary["weekly_volume"],
                "friend": friend_summary["weekly_volume"],
            },
            {
                "metric": "Workout Streak",
                "you": user_summary["workout_streak"],
                "friend": friend_summary["workout_streak"],
            },
            {
                "metric": "Max PR",
                "you": user_summary["max_pr"],
                "friend": friend_summary["max_pr"],
            },
            {
                "metric": "Total Workouts",
                "you": user_summary["total_workouts"],
                "friend": friend_summary["total_workouts"],
            },
        ],
